...
class openligaserverclass {
    ...
    */
    ...
    constructor(adapter) {
        ...
        this.ioUtil.loginfo('Initializing OpenLigaDB Server');
        this.init();
    }
    /**
    ...
    calcCurrentGameDay(allmatches, currentGameDay) {
        let startDate = { matchDateTime: '0', groupOrderID: 1 };
        if (allmatches.length == 0) {
            return 1;
        }
        if (allmatches.length == 1) {
            return allmatches[0].group.groupOrderID;
        }
        if (allmatches.length > 1) {
            startDate = { matchDateTime: allmatches[0].matchDateTime, groupOrderID: allmatches[0].group.groupOrderID };
        }
        let firstPlayDate = allmatches.reduce((previousValue, currentValue) => {
