...
(() => {
  ...
  // openligadb/js/openligadb.js
  var require_openligadb = __commonJS({
    "openligadb/js/openligadb.js"(exports) {
      init_package();
      fetch("widgets/openligadb/i18n/translations.json").then((res) => __async(null, null, function* () {
...
`;
            ...
            table.forEach(function(team1) {
              ...
              if (vis.binds["openligadb"].checkHighlite(
                team1.teamName,
...
`;
          ...
          filterFavGames: function(allmatches, gameday, gamedaycount, currgameday, filter, shortname, abbreviation) {
            ...
            return allmatches.reduce(function(result, item) {
              ...
              if (gameday > 0 && item.group.groupOrderID >= gameday && item.group.groupOrderID < gameday + gamedaycount) {
                found = item;
              }
              if (gameday < 0 && item.group.groupOrderID >= currgameday + gameday && item.group.groupOrderID < currgameday + gameday + gamedaycount) {
                found = item;
              }
              item.filter = filter;
              item.shortname = shortname;
              if (found && (vis.binds["openligadb"].checkHighlite(
                item.team1.teamName,
          ...
          sortFavGames: function(favgames) {
            return favgames.sort(function(a, b) {
              let comp = 0;
              if (vis.binds["openligadb"].getDateFromJSON(a.matchDateTime).getTime() > vis.binds["openligadb"].getDateFromJSON(b.matchDateTime).getTime()) {
                comp = 1;
              }
              if (vis.binds["openligadb"].getDateFromJSON(a.matchDateTime).getTime() < vis.binds["openligadb"].getDateFromJSON(b.matchDateTime).getTime()) {
                comp = -1;
            ...
            if (json && json != "null") {
              ...
              if (matches) {
                ...
                return matches.reduce(function(result, item) {
                  ...
                  const test = vis.binds["openligadb"].compareDate(today, mdate) && (vis.binds["openligadb"].checkHighlite(item.team1.teamName, highlite, ",") || vis.binds["openligadb"].checkHighlite(item.team2.teamName, highlite, ","));
                  return result || test;
                }, false);
              }
            } else {
